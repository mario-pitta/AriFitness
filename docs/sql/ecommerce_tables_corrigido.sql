-- =============================================================================
-- Tabelas de E-commerce para Academias (Corrigido)
-- =============================================================================
...
-- -----------------------------------------------------------------------------
-- Tabela de Pedidos (corrigido: cliente_id agora bigint)
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS pedidos (
    ...
    -- Cliente (agora bigint para compatível com usuarios.id)
...
-- -----------------------------------------------------------------------------
-- View para pedidos com resumo de itens
-- -----------------------------------------------------------------------------
...
SELECT 
    ...
    SUM(CASE WHEN status = 'pago' THEN valor_total - COALESCE(valor_desconto, 0) ELSE 0 END) as receita_total,
    AVG(CASE WHEN status = 'pago' THEN valor_total - COALESCE(valor_desconto, 0) END) as ticket_medio,
    COUNT(DISTINCT DATE(created_at)) as dias_com_vendas
FROM pedidos
WHERE fl_ativo = true
GROUP BY empresa_id;
...
BEGIN
    ...
    -- Criar pedido
    INSERT INTO pedidos (empresa_id, cliente_id, cliente_nome, cliente_telefone, forma_pagamento, valor_total, valor_desconto)
    VALUES (p_empresa_id, p_cliente_id, p_cliente_nome, p_cliente_telefone, p_forma_pagamento, 0, p_valor_desconto)
    ...
    UPDATE pedidos SET valor_total = v_valor_total - p_valor_desconto WHERE id = v_pedido_id;
